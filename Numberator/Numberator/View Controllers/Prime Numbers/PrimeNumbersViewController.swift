...
class PrimeNumbersViewController: UIViewController {
    ...
    private let calculator = PrimeCalculator()
    ...
    override func viewDidLoad() {
        ...
        calculateNumbers(startFrom: 2) { [weak self] result in
    ...
    private func calculateNumbers(startFrom: Int, completion: @escaping (([Int]) -> Void)) {
        guard !isCalculating else { return }
        isCalculating = true
        calculator.calculate(count: 30, startFrom: startFrom, completion: { [weak self] result in
                                guard let self = self else { return }
...
extension PrimeNumbersViewController: UICollectionViewDelegateFlowLayout, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.configure(with: Decimal(numbers[indexPath.row]), color: color)
    ...
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        if indexPath.row >= numbers.count - 5 {
            calculateNumbers(startFrom: numbers.last ?? 2) { [weak self] result in
                guard let self = self else { return }
